...
export default function Predict(props) {
  ...
  const handlePredict = () => {
    ...
    var CUSTOMERORDERIDdata = {
      data: props.CUSTOMERORDERIDsend,
    };
    return fetch(`http://127.0.0.1:5000/get_prediction`, {
      ...
      body: JSON.stringify(CUSTOMERORDERIDdata),
    })
  ...
  const handlePredictDataUpdate = () => {
    for (var i = 0; i < storeData.length; i++) {
      CUSTOMER_ORDER_ID.push(storeData[i].CUSTOMER_ORDER_ID);
    }
    console.log(CUSTOMER_ORDER_ID);
    try {
      axios
        .post(
          `http://localhost:8081/backend/PredictServlet?CUSTOMER_ORDER_ID=${CUSTOMER_ORDER_ID}`,
        )
      ...
      >
        ...
        <DialogActions className={classes.divs}>
          <Button
            ...
            onClick={handlePredictDataUpdate}
            color="primary"
          >
            Close
          </Button>
